...
/**
 * A helper class containing methods that almost all {@code CommandFactory}s will use. Provide helpers that wait for
 * and convert text input, as well as a constructor which ensures non-null values.
 */
public abstract class AbstractCommandFactory implements CommandFactory {
    ...
    /**
     * If a {@code hadPreviousValue} is true, this method waits for input from the user. If there is no remaining input,
     * then a message is sent to the view that says the program reached the end of input. If {@code hadPreviousValue} is
     * false, then no message is sent to the view. If the method begins waiting for user input and the user enters a
