...
/**
 * A helper factory for commands that change all pixels by some intensity.
 * Handles input conversion for subclasses so that implementors only
 * need to map the arguments to the command constructors.
 */
public abstract class AbstractShadeChangeFactory extends AbstractCommandFactory {
  /**
   * Creates a new AbstractShadeChangeFactory with the given view and scanner.
  ...
  /**
   * Waits for a string input, an integer input, and a string input and creates
   * a command given with the given parameters. If the user enters "q" or "quit"
   * (case-insensitive) for any of these parameters, this method returns an empty
   * {@code Optional}. Otherwise, if the user enters a malformed integer, this
   * method asks the user to re-enter a valid value. If the end of input is reached,
   * a message is sent to the view and an empty {@code Optional} is returned.
   *
  ...
  public Optional<Command> make() {
    ...
    Optional<Integer> intensity = waitForInteger(imageName.isPresent(),
            "Expected integer for intensity");
  ...
  /**
   * Creates a command following the (string, integer, string) constructor pattern,
   * passing in the respective arguments.
   *
